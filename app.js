...
function exportDataQuestionnaireCSV() {
  const table = document.querySelector('#admin-data-report .card:last-child table');
  if (!table) return toast('No questionnaire analytics to export', 'warning');
  const headers = [...table.querySelectorAll('thead th')].map(cell => cell.textContent.trim());
  const rows = [...table.querySelectorAll('tbody tr')].filter(row => row.cells.length === headers.length).map(row => [...row.cells].map(cell => cell.textContent.trim()));
  if (!rows.length) return toast('No questionnaire analytics to export', 'warning');
  downloadCSV(headers, rows, 'questionnaire-wise-analytics.csv');
}
