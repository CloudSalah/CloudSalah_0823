...
function profileFormBody(roles, members, u = null) {
  ...
  return `
    ...
    <div class="form-group"><label>User Name</label>
      <input id="mEmail" type="text" placeholder="user name">
    </div>
    <div class="form-group">
      <label>${u ? 'New Password <span style="font-weight:400;color:#9ca3af">(leave blank to keep)</span>' : 'Password'}</label>
      <input id="mPassword" type="password" placeholder="Min 6 characters">
    </div>`;
}
...
async function showDonationModal() {
  ...
  async () => {
    ...
    const { error } = await supa.from('donation_records').insert({
      ...
      donor_phone: val('mDonorPhone') || null,
      donor_city: val('mDonorCity') || null,
      donor_state: val('mDonorState') || null,
      ...
      collection_date: val('mDonationDate') || null,
      notes: val('mDonationNotes') || null,
    });
    if (error) {
